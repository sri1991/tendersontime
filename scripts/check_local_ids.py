...
client = chromadb.HttpClient(host='136.114.154.210', port=8000)
...
try:
    print(f"Connecting to local ChromaDB at ./chroma_db...")
    ...
    # Analyze found items to see why they might be missed in search (e.g. metadata content)
    print("\n--- Analyzing Found Items ---")
    for i in range(min(3, len(found_ids))):
         ...
         print(f"Corrigendum: {meta.get('is_corrigendum')}")
